...
def updaterating(league):
  for team in league:
    ...
    for player in league[team]["squad"]:
      ...
      if position == "A":
        database[player][0] += rchan + radj()
        database[player][1] += rchan + radj() 
        database[player][3] += rchan + radj()
      elif position == "M":
        database[player][2] += rchan + radj()
        database[player][5] += rchan + radj()
      elif position == "D":
        database[player][4] += rchan + radj()
        database[player][5] += rchan + radj()
      for stat in range(len(database[player])-2):
        if database[player][stat] < 20:
          database[player][stat] = 20
